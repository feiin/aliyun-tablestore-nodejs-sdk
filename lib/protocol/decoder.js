...
TableStore.decoder = {
    ...
    decodeListStream: function (buffers) {

        var response = tsProtos.ListStreamResponse.decode(buffers);
        return response;
    },
    decodeDescribeStream: function (buffers) {

        var response = tsProtos.DescribeStreamResponse.decode(buffers);
        return response;
    },
    decodeGetShardIterator: function (buffers) {

        var response = tsProtos.GetShardIteratorResponse.decode(buffers);
        return response;
    },
    decodeGetStreamRecord: function (buffers) {

        var response = tsProtos.GetStreamRecordResponse.decode(buffers);
        for (let row of response.stream_records) {
            let inputStream = new TableStore.PlainBufferInputStream(row.record);
            let codeInputStream = new TableStore.PlainBufferCodedInputStream(inputStream);

            row.record = codeInputStream.readRow();
        }
        return response;
    },
...
var tsMap = {
    ...
    'listStream': TableStore.decoder.decodeListStream,
    'describeStream': TableStore.decoder.decodeDescribeStream,
    'getShardIterator': TableStore.decoder.decodeGetShardIterator,
    'getStreamRecord': TableStore.decoder.decodeGetStreamRecord,
