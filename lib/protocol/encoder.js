...
TableStore.encoder = {
    ...
    encodeListStream: function (params) {
        var request = new tsProtos.ListStreamRequest();
        request.table_name = params.table_name;
        return request;
    },
    encodeDescribeStream: function (params) {
        var request = new tsProtos.DescribeStreamRequest();
        request.stream_id = params.stream_id;
        if(params.inclusive_start_shard_id) {
            request.inclusive_start_shard_id = params.inclusive_start_shard_id;

        }
        if(params.shard_limit) {
            request.shard_limit = params.shard_limit;
        }

        return request;
    },
    encodeGetShardIterator: function (params) {
        var request = new tsProtos.GetShardIteratorRequest();
        request.stream_id = params.stream_id;
        request.shard_id = params.shard_id;
        return request;
    },
    encodeGetStreamRecord: function (params) {
        var request = new tsProtos.GetStreamRecordRequest();
        request.shard_iterator = params.shard_iterator;
        if(params.limit) {
...
var tsMap = {
    ...
    'listStream':TableStore.encoder.encodeListStream,
    'describeStream':TableStore.encoder.encodeDescribeStream,
    'getShardIterator':TableStore.encoder.encodeGetShardIterator,
    'getStreamRecord':TableStore.encoder.encodeGetStreamRecord,
};
