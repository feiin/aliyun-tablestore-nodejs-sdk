...
TableStore.PlainBufferCodedInputStream = inherit({
    ...
    readRowExtension:function() {

        if (!this.checkLastTagWas(TableStore.plainBufferConsts.TAG_EXTENSION)) {
            throw new Error("Expect TAG_EXTENSION but it was " + this.getLastTag());
        }

        this.inputStream.readRawLittleEndian32();
        this.readTag();

        if (!this.checkLastTagWas(TableStore.plainBufferConsts.TAG_SEQ_INFO)) {
            throw new Error("Expect TAG_SEQ_INFO but it was " + this.getLastTag());
        }
        this.inputStream.readRawLittleEndian32();
        this.readTag();

        if (!this.checkLastTagWas(TableStore.plainBufferConsts.TAG_SEQ_INFO_EPOCH)) {
            throw new Error("Expect TAG_SEQ_INFO_EPOCH but it was " + this.getLastTag());
        }

        let epoch = this.inputStream.readRawLittleEndian32();
        this.readTag();

        if (!this.checkLastTagWas(TableStore.plainBufferConsts.TAG_SEQ_INFO_TS)) {
            throw new Error("Expect TAG_SEQ_INFO_TS but it was " + this.getLastTag());
        }

        let ts = this.inputStream.readRawLittleEndian64();

        this.readTag();

        if (!this.checkLastTagWas(TableStore.plainBufferConsts.TAG_SEQ_INFO_ROW_INDEX)) {
            throw new Error("Expect TAG_SEQ_INFO_ROW_INDEX but it was " + this.getLastTag());
        }

        let rowIndex = this.inputStream.readRawLittleEndian32()

        let ext = {epoch, ts, rowIndex};
        return ext;
    },
    ...
    readRowWithoutHeader: function () {
        ...
        if (!this.checkLastTagWas(TableStore.plainBufferConsts.TAG_ROW_PK)) {
            throw new Error("Expect TAG_ROW_PK but it was " + this.getLastTag());
        }
        this.readTag();
        ...
        if (this.checkLastTagWas(TableStore.plainBufferConsts.TAG_EXTENSION)) {
            extension = this.readRowExtension();
            this.readTag();
        }
        ...
        return { primaryKey: primaryKey, attributes: attributes, extension: extension };
    },
...
TableStore.PlainBufferCodedOutputStream = inherit({
    ...
    writePrimaryKeyValue: function (value, cellCheckSum) {
        ...
        if (value === TableStore.INF_MIN) {
            ...
            cellCheckSum = TableStore.plainBufferCrc8.crcString(cellCheckSum, binary_value.toString());
        } else {
